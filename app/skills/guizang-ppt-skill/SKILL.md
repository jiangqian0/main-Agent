...
### Step 1 · 需求对齐（流式聊天环境适配）

> ⚠️ **重要**：本 skill 已适配流式聊天环境，请仔细阅读以下规则。

**在流式聊天环境中，一次性处理所有输入，不要多次往返**：
1. 如果用户已经给了**完整的信息**（风格 + 大纲/主题 + 内容），直接进入 Step 2
2. 如果用户只给了**部分信息**，一次性问 1-3 个最关键的问题（不要逐个问）
3. 如果用户没有给某些信息，使用**合理默认值**并告知用户

**认可用户已说的内容**：当用户明确说"风格A"或"瑞士风"时，直接认可这个选择，不需要再问"你想要哪种风格"。

#### 7 项需求清单（不需要逐个问，用到的才确认）

| # | 项目 | 默认值 | 何时确认 |
|---|------|--------|----------|
| 1 | **风格 A 还是 B?** | 风格 A（电子杂志风） | 用户没说时才问 |
| 2 | **受众/场景** | 行业内部分享 | 用户没说就默认 |
| 3 | **分享时长** | 15 分钟（≈10 页） | 用户没说就默认 |
| 4 | **原始素材** | 无，用户口述/描述 | 根据用户提供的内容填充 |
| 5 | **图片/截图** | 无，先用占位色块 | 用户有图才处理 |
| 6 | **主题色** | 墨水经典（通用） | 用户没说就默认 |
| 7 | **硬约束** | 无 | 用户没说就无 |
...
✅ 正确示范（一次性处理）：
用户："帮我做一个关于AI的英语演讲PPT，风格A"
→ 认可"风格A"，问1-2个最关键问题：
"好的，风格A。我来帮你做这个英语演讲PPT。
请问：
1. 演讲时长大约多久？（默认15分钟≈10页）
2. 有现成的演讲稿或大纲吗？还是我根据主题帮你搭建结构？"
...
2. 确认风格 A 还是 B（用户已说的直接认可，没说的默认风格 A）
